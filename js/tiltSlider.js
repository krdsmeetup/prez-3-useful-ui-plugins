...
;( function( window ) {
	...
	TiltSlider.prototype._initEvents = function() {
		...
		this.navDots.forEach( function( dot, idx ) {
			dot.addEventListener( 'click', function() {
				if( idx !== self.current ) {
					...
					sliderBusy = true;
					self._showItem( idx );
					setTimeout(function(){ sliderBusy = false; }, 2500);
				}
			} );
		} );
		...
		targetElement.addEventListener('keydown', function (event) {
			var direction,
			keyCode = event.keyCode, slideToShow;
			...
			if (isArrowKey(keyCode)) {
				...
				if(direction == 'right' || direction == 'left') {
					...
					sliderBusy = true;
					slideToShow = (direction == 'right') ? self.current + 1 : self.current - 1;
					self._showItem( slideToShow );
					setTimeout(function(){ sliderBusy = false; }, 2500);
				}
			}
		});
